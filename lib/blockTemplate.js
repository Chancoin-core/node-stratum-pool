...
**/
var BlockTemplate = module.exports = function BlockTemplate(jobId, rpcData, poolAddressScript, extraNoncePlaceholder, reward, txMessages, recipients, algorithm, height){
    ...
    this.serializeHeader = function(merkleRoot, nTime, nonce){
        ...
        var header =  new Buffer((this.algorithm == "nightcap") ? 100 : 80);
        var position = 0;
        if(this.algorithm == "nightcap") {
            header.writeUInt32BE(this.height, position, 4);
            header.write('00000000000000000000000000000000', position += 4, 16, 'hex');
            position += 16;
        }
        ...
        var headerr = util.reverseBuffer(header);
        var cmix = multiHashing.nightcapcmix(headerr);
        header.write(util.reverseBuffer(cmix).toString('hex'), 4, 16, 'hex');
